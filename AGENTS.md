...
1. 機械チェック:
   ...
   - 壊れた`[[wikilink]]` — 参照数で分類:
     - **1ページのみ参照 = BROKEN**: typoまたは意図的赤リンク。typoなら修正、意図的なら放置（ページを作らない）
     - **2+ページで参照 = ASPECT_HANDLE**: 概念ハブの種。エラーではなく、ページ作成の候補として人間に提案
   - index.md に載っていないページ
   - indexエントリが指すファイルが存在しない（stale） — ページ削除・リネーム後のindex残骸。陳腐化時はページを読み直して再生成してよい
...
3. 新しい調査課題と探すべきソースを提案する。ingest時の未カバー領域指摘（step 2）が溜まっている場合はそれも材料にする。
