...
- [ ] 配布形式はどうするか？（別リポジトリ / `template/` ディレクトリ / GitHub Template Repository / `npx create-llm-wiki` / その他）
  ...
  - 2026-08-14: 操作層の配布構成 — **v0.1は二層構成（AGENTS.md + `.agents/skills/` 同梱（手順と決定スクリプト））**（コピーだけで動く・Claude Codeのみパス参照1行）。lint.shはskill同梱（2026-08-14改訂: root `scripts/` → `.agents/skills/llm-wiki-lint/scripts/`。[[MCP不採用とAGENTS.md+SKILL.md二層採用の根拠]]）。GitHub Template RepositoryはM3（別repo切り出し）の候補に繰り下げ。v0.1スコープ決定（二層・対象範囲・規範のみ原則）は [[claims対応表]] と [[配布形式の決定とテンプレートの動機]]
...
- [[overview]] — エントリーポイント（[[overviewの役割]] — その位置づけ・原型との関係の分析）
- [[index]] — ページカタログ
